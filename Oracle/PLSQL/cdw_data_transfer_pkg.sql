...
CREATE OR REPLACE PACKAGE BODY cdw_data_transfer_v3 AS
    ...
    Begin
       ...
       Exception
          ...
          When Others then
              ...
              err_msg := substr(sqlerrm,1,100);
              If Err_Num = -1422 Then
                 Log_Util.LogMessage('FNDLABMAP - ERROR: Study = "'||i_Studyid||'" has duplicate Control Record (NCI_LAB_LOAD_CTL).');
              Else
    ...
    Else
       ...
       If Upper(P_Method) in ('MARK','PROCESS') Then  -- prc 06/14/04
          Update NCI_LABS
                ...
                where load_flag = 'E'
                  and oc_study is not null
                  and oc_study like E_Study  -- prc 10/26/04 added "nvl" function
                  and patient_id like E_Patientid
                  and error_Reason like E_Reason; -- sy
 ...
 End;   -- Process_Error_Labs
  ...
  Procedure Update_After_Batch_Load is
    ...
    /* Author: Patrick Conrad- Ekagra Software Technologies                              */
    /*   Date: 08/08/13                                                                  */
    /*    Mod: Added Global Variable to hold the current processing status such that this*/
    /*         procedure can identify the last status process and check that status only */
    /*         Also reverted the update statement for "L" status only as the FOR LOOP    */
    /*         approach does not execute quickly for LARGE data sets.                    */
    /* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
    ...
    Case Status$Flag 
       ...
       ELSE 
          Begin
             ...
             Update_STATUS_FLAG('D','U','Record loaded as update.','"D" records failed to be verified as loaded as update.');
             Update_STATUS_FLAG('W', 'U', 'Record loaded as update.','"W" records failed to be verified as loaded as update.');
          End;
    End Case;
   ...
   Begin
      ...
      update nci_labs N
       ...
       where load_flag = 'L'
         ...
         and not exists (select 'X' from NCI_DFACT_DCM_QST_REPDEF_VW  a
       ...
       UPDATE NCI_LABS N
        ...
        WHERE LOAD_FLAG = 'N' AND OC_LAB_QUESTION IS NOT NULL AND
              ...
              (SELECT DISTINCT V.DCM_NAME || ',' || V.OC_LAB_QUESTION
                 FROM CLINICAL_STUDIES S, NCI_STUDY_DCMS_VW_TB V

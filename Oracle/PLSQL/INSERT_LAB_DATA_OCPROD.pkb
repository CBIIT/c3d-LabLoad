CREATE OR REPLACE PACKAGE BODY insert_lab_data
AS
...
/* Replace SLOW version of cursor with below definition
CURSOR c1 is
...
Cursor C1 is
   SELECT distinct v.repeat_sn repeat_seq,
          substr(v.oc_lab_question,1,20) value_text
     from nci_study_labdci_quests_vw v
    WHERE v.study = ipstudy
        AND V.dcm_name = ipdcm_name
        AND V.subset_name = dcm_subset
        AND V.QUESTION_NAME = 'LPARM'
        AND V.REPEAT_SN NOT IN (SELECT repeat_sn FROM BDL_RESPONSE_REPEATS)
...
/*  01/02/07:PRC : Added Question Layout sequence and Question Subset Sequences to   */
/*                 primary Cursor "C1".  Certain Studies may have multiple question  */
/*                 layouts or subsets which would cause a cartesian product when they*/
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
   ...
   select ri.CLIN_PLAN_EVE_NAME PLANNED_EVENT,
    ...
    where ri.received_dci_status_code='BATCH LOADED'
      ...
      and dq.DCM_QUE_DCM_LAYOUT_SN=d.DCM_LAYOUT_SN -- Added Question Layout
      and dq.DCM_QUE_DCM_SUBSET_SN=d.DCM_SUBSET_SN -- Added Question subset

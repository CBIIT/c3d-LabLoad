CREATE OR REPLACE PACKAGE BODY cdw_data_transfer_v3 AS
     ...
     If i_Lab_Code = 'CDW' Then
        Begin
           SELECT DISTINCT M.OC_LAB_QUESTION
               ...
               WHERE M.Map_VERSION = Find_LabMap_Version(i_StudyID)  -- Added Lab Map Version
              ...
              WHen No_Data_Found then
                 Begin
                    select distinct OC_LAB_QUESTION
                     ...
                     WHERE Map_VERSION = Find_LabMap_Version(i_StudyID)   -- Added Lab Map Version
  ...
  Function Find_LabMap_Version(i_StudyID in Varchar2) Return Varchar2
  is
  /* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
  /*     Author: Patrick Conrad - Ekagra Software Technologies                         */
  /*       Date: 04/24/2007                                                            */
  ...
  Begin
     ...
     Exception
        ...
        When Others then
            ...
            Else
               Log_Util.LogMessage('FNDLABMAP - Error during FIND_LAB_QUESTION');
  ...
  Begin
     ...
     For x_Rec in (select a.oc_study, a.load_other_labs
        ...
        If X_Rec.load_other_labs = 'Y' Then
           ...
           Log_Util.LogMessage('PLD - '||to_char(SQL%RowCount)||' rows successfully updated "OC_LAB_PANEL", "OC_LAB_EVENT", "OC_LAB_SUBSET"'||
                              ' with LAB_ALL, OTHER LABS and LABA for Study '||X_Rec.OC_Study);

        Else

           UPDATE NCI_LABS
              SET LOAD_FLAG = 'E',
                  ERROR_REASON = 'Study '||X_Rec.OC_Study||' does not load "Other Labs".'
            WHERE oc_study = X_Rec.OC_Study
              and LOAD_FLAG = 'N'
              and oc_lab_panel is null
              AND oc_lab_subset is null
 ...
 End;   -- Process_Error_Labs
  ...
  Procedure Update_After_Batch_Load_OLD is
    /* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
    /*     Author: Patrick Conrad - Ekagra Software Technologies                         */
    /*       Date: 07/10/2003                                                            */
    /*Description: This procedure is used to identify labs that were recently loaded, but*/
...
Procedure Check_SubEvent_Numbers is
  ...
  Begin
     ...
     If Instr(v_Process_Type,'WAIT_PROC_MARK_LOAD(',1) = 1 Then
        ...
        v_autoLoad_Study := Rtrim(LTRIM(v_Process_Type,'WAIT_PROC_MARK_LOAD('),')');
     ...
     If v_Process_Type in ('GET_PROC','FULL', 'WAITING','WAIT_PROC', 'GET_PROC_MARK','WAIT_PROC_MARK') Then
        ...
        Log_Util.LogMessage('GPLL - '||to_char(SQL%RowCount)||' records found "Holding" and reset.');
  ...
  Begin
     ...
     Log_Util.LogMessage('APTS - Assign Patiets to Studies: BEGIN');
     ...
     Log_Util.LogMessage('APTS - '||to_char(SQL%RowCount)||' rows successfully set "oc_patient_pos" and "oc_study"');

     Log_Util.LogMessage('APTS - Assign Patiets to Studies: DONE');
  ...
  Procedure Identify_Additional_Labs_OLD2 is
  /* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
  /* This section is used to added additional records to NCI_LABS for processing */
  /* by identifying new study/patients relationships for existin patients.  Also */
  /* resets those errors held in the automatic error reset control table.        */
  ...
  Begin
     ...
     Log_Util.LogMessage('IAL - Generated additional '||to_char(X_Cnt)||
                             ' lab records for patients on more than one study, or on same study more than once.');
  End Identify_Additional_Labs_Old2;
  ...
  Begin
     ...
     Begin
        ...
        Log_Util.LogMessage('IAL - Generated additional '||to_char(T_Cnt)||' lab records for patients on more '||
                                  ' than one study, or on same study more than once.');
     End;
    ...
    BEGIN
       ...
       For I_Rec in (select distinct Patient_id, OC_Study, OC_Patient_Pos
          ...
          If v_date_check_code in ('BOTH','PRE') Then
             ...
             Else
                --Log_Util.LogMessage('prestudy_lab_date="'    ||to_char(prestudy_lab_date)||'";  ');
                --Log_Util.LogMessage('v_prestudy_offset="'    ||to_char(nvl(v_prestudy_offset,'!'))||'";  ');
                --Log_Util.LogMessage('I_Rec.OC_PATIENT_POS="' ||I_Rec.OC_PATIENT_POS||'";  ');
                --Log_Util.LogMessage('I_Rec.OC_Study="'    ||I_Rec.OC_Study||'";  .');
       ...
       For Dup_Lab in (SELECT distinct V.STUDY, M.test_component_id,
                               ...
                               WHERE V.DEFAULT_VALUE_TEXT = M.OC_LAB_QUESTION
                                 ...
                                 and m.map_version        = find_labmap_version(n.oc_study)
       ...
       */
       ...
       /* The following 2 Statements are not needed now AS the ABOVE 2 Statements use
       /* the new Study Control View which creates a control record with the default "ALL" study values
       /* applied for missing values and missing studies.

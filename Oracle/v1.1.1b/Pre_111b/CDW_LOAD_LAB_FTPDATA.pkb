CREATE OR REPLACE PACKAGE BODY OPS$BDL.cdw_load_lab_FTPdata AS
  /* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
  ...
  /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  /* 12/06/2005:PRC: Replaced reference to patient_id_ptid_vw view with new table      */
  /*                 NCI_LAB_VALID_PATIENTS.  New table is populated by Lab Loader with*/
  /*                 only valid lab patients.                                          */
  /* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
...
Begin
   If Instr(In_String,Chr(9)) = 0 Then
      x_temp := substr(rtrim(In_String,chr(13)),1);
      In_String := Null;
   Else
...
Begin
   ...
   select distinct to_char(Patient_id), 'X'

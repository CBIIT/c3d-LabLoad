...
CREATE OR REPLACE PACKAGE BODY cdw_data_transfer_v3 AS
 ...
 End;   -- Process_Error_Labs
  ...
  Begin
    ...
    -- prc 06/21/06: Removed question.  It only matters if the sample date was previously loaded, not the 
    --               question.  This way, if a Repeat Default is added, and it needs loaded, it will load as
     ...
     where load_flag = 'L'    -- Change N to L, because of LLI Processing.
       and exists
              (SELECT n.PATIENT_ID, n.LABTEST_NAME, n.RESULT, n.sample_datetime,
                ...
                -- AND dp.QUESTION_NAME='LPARM' -- prc 06/21/06 : Removed question. See above
